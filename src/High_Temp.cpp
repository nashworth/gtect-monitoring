...
{
    long sum = 0;
    for(int i=0; i<32; i++)         // take 32 samples of the analog pin
    {
        sum += analogRead(pin);
    }
...
{
    int a = getAnalog(pinRoomTmp);                                      // read the analog value of room temp
    float resistance=(float)(4095-a)*10000/a;                           // calculate resistance
    float temperature=1/(log(resistance/10000)/10000+(1/298.15))-273.15;   // convert to temperature via datasheet ;
...
//    float vout = (float)getAnalog(pinThmc)/1023.0*5.0*1000;
    float vout = (float)getAnalog(pinThmc)/4095.0*3.3*1000;             // modified for 12 bit A2D and 3.3V Supply
